  ...
  return (
    <section className="border-b border-border py-14 sm:py-20 md:py-24">
      <Container>
        ...
        <ol className="mt-12 grid grid-cols-1 gap-px overflow-hidden rounded-[16px] border border-border bg-border sm:grid-cols-2 lg:grid-cols-4">
          {steps.map((step, i) => (
            <li key={step.title} className="bg-surface">
              <Reveal delay={Math.min(i * 0.06, 0.3)} className="block h-full">
                <div className="flex h-full flex-col p-6 sm:p-7">
                  <span className="font-display text-[40px] leading-none tracking-[-0.04em] text-accent/80 tabular-nums">
